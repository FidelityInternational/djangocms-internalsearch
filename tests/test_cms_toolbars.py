...
from djangocms_internalsearch.models import PageInternalsearch
...
class ExtendedPageToolbarTest(BaseToolbarTest):
    ...
    def test_show_publish_button_if_Internalsearch_disabled_for_page(self):
        self.wf1.is_default = False
        self.wf1.save()  # making all workflows not default, therefore by design Internalsearch is disabled
    ...
    def test_show_Internalsearch_dropdown_if_Internalsearch_request(self):
    ...
    def test_show_Internalsearch_dropdown_with_no_actions_for_non_role_user(self):
    ...
    def test_show_submit_for_Internalsearch_button_if_page_is_dirty(self):
        new_page = create_page(title='New Page', template='page.html', language='en', published=True,)
        self.setup_toolbar(new_page, self.user)
        buttons = sum([item.buttons for item in self.toolbar_right_items if isinstance(item, ButtonList)], [])
        self.assertTrue([button for button in buttons if force_text(button.name) == 'Submit for Internalsearch'])

    def test_submit_for_Internalsearch_button_with_default_settings(self):
        new_page = create_page(title='New Page', template='page.html', language='en', published=True,)
        self.setup_toolbar(new_page, self.user)
        buttons = sum([item.buttons for item in self.toolbar_right_items if isinstance(item, ButtonList)], [])
        submit_for_Internalsearch_button = [
            button for button in buttons if force_text(button.name) == 'Submit for Internalsearch'
        ][0]
        url = get_admin_url(
        ...
        self.assertEqual(submit_for_Internalsearch_button.url, url)

    @patch('djangocms_internalsearch.conf.ENABLE_WORKFLOW_OVERRIDE', True)
    def test_submit_for_Internalsearch_button_with_override_settings(self):
        new_page = create_page(title='New Page', template='page.html', language='en', published=True,)
        self.setup_toolbar(new_page, self.user)
        buttons = sum([item.buttons for item in self.toolbar_right_items if isinstance(item, ButtonList)], [])
        submit_for_Internalsearch_button = [
    ...
    def test_publish_button_after_Internalsearch_request_approved(self):
        self.setup_toolbar(self.pg3, self.user)  # pg3 => Internalsearch request is approved
...
class PageInternalsearchToolbarTest(BaseToolbarTest):

    def test_Internalsearch_menu_add_rendered(self):
        new_page = create_page(title='New Page', template='page.html', language='en',)
        self.setup_toolbar(new_page, self.user)
        page_menu = self.toolbar.menus['page']
        opts = PageInternalsearch._meta
        url = get_admin_url(
    ...
    def test_Internalsearch_menu_change_rendered(self):
        new_page = create_page(title='New Page', template='page.html', language='en',)
        extension = PageInternalsearch.objects.create(extended_object=new_page, enabled=True, workflow=self.wf1,)
        self.setup_toolbar(new_page, self.user)
        page_menu = self.toolbar.menus['page']
        opts = PageInternalsearch._meta
        url = get_admin_url(
